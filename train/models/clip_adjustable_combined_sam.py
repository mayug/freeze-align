...
class CLIP(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        print('lcoal text ', self.local_text_proj)
        print('text ', self.text_proj)
    ...
    def _forward_(self, image, text, return_dict=False, return_pooled_output=False):
        ...
        if image is not None:
            image_embeds = self.visual_encoder.forward_image(image)['vision_features']
            bs , nc, h, w = image_embeds.shape
            image_embeds = image_embeds.reshape(bs, nc, h*w).permute(0, 2, 1)
        else:
            image_embeds = None
    ...
    def _get_features_(self, image_embeds, text_embeds, text, text_pooled_output=None):
        ...
        if image_embeds is not None:
            image_pooled_output = image_embeds.mean(1)
