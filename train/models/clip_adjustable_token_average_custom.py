...
class CLIP(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        print('This is the CLIP model from models/clip_adjustable_token_average_custom.py')
        # asd
    ...
    def _get_features_(self, image_embeds, text_embeds, text):
        ...
        if text_embeds is not  None:
            ...
            text_embeds = text_embeds * text['attention_mask'].unsqueeze(-1)
            # print('text_embeds ', text_embeds.shape)
            text_embeds = text_embeds.sum(dim=1) / text['attention_mask'].sum(dim=1).unsqueeze(-1)
            # print('text_embeds ', text_embeds.shape)
            # asd
