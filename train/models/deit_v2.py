...
class Layer_scale_init_Block(nn.Module):
    ...
    def forward(self, x):
        x = x + self.drop_path(self.gamma_1 * self.attn(self.norm1(self.adapter1(x))))
        x = x + self.drop_path(self.gamma_2 * self.mlp(self.norm2(self.adapter2(x))))
        return x
...
class Layer_scale_init_Block_paralx2(nn.Module):
    ...
    def __init__(
        ...
        if self.insert_adapter:
            self.adapter1 = Adapter(adapter_config)
            self.adapter11 = Adapter(adapter_config)
            self.adapter2 = Adapter(adapter_config)
            self.adapter21 = Adapter(adapter_config)
        else:
            self.adapter1 = nn.Identity()
            self.adapter2 = nn.Identity()
            self.adapter11 = nn.Identity()
            self.adapter21 = nn.Identity()

    def forward(self, x):
        x = (
            x
            + self.drop_path(self.gamma_1 * self.attn(self.norm1(self.adapter1(x))))
